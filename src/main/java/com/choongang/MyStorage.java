...
//                counts[i] = 0;  //불필요
    ...
    static void prod_remove(Scanner s){
        ...
        System.out.print("[System] 제품 취소가 완료됬습니다.");
    }
...
//        while(cntInput==null);
        ...
        int cnt = Integer.parseInt(cntInput);  // 수량 입력 받기
    ...
    public static void prod_amount_decrease(Scanner s) {
        ...
        String prod = s.nextLine();
        for (; currentIdx < products.length; currentIdx++){
            if(products[currentIdx].equals(prod)){
                break;
            }
        }
...
//        do{
            ...
            if(cntInput == null) {
                ...
                outAmount = Integer.parseInt(cntInput);
                if (counts[currentIdx] < outAmount) {
                    System.out.println("[Error] 남은 수량보다 더 많이 출고할 수 없습니다.");
                    return;
                }
            }
//        }
//        while(cntInput==null || counts[currentIdx] < outAmount);


        counts[currentIdx] -= outAmount;
    ...
    static String value_integer_chk(Scanner s) { //유효성 체크
        String input = s.nextLine();
        String num = "0123456789";
        for(int i = 0; i < input.length(); i++){
            if(num.indexOf(input.charAt(i)) == -1)
                return null;
        }
        return input;
    }
